@testset "Fourier Stokes assembly" begin
    ...
    for (Nx, Nz) in ((6, 6), (5, 5), (6, 5), (5, 6), (1, 5), (5, 1), (1, 1), (2, 2)),
        ...
        @testset "Nx=$Nx, Nz=$Nz, base mean=$base_mean" begin
            grid = Grid(Ny, Nx, Nz, Lx, Lz, profile)
            ...
            for fixedbulk in (false, true)
                ...
                actual = fixedbulk ? solve!(solver, U, P, R; bulkvelocity=(base_mean+0.2*2/3, -0.1*2/3)) :
    ...
    @test_throws ArgumentError FourierStokesSolver(Grid(10, 5, 5, Lx, Lz, y -> y), nu, lambda)
    @test_throws ArgumentError FourierStokesSolver(Grid(Ny, 0, 5, Lx, Lz, y -> y), nu, lambda)
    @test_throws ArgumentError FourierStokesSolver(Grid(Ny, 5, 5, 0, Lz, y -> y), nu, lambda)
end
